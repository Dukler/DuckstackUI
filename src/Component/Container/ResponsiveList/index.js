...
const createItemSizes = memoize((sectionCount, data, itemSize) =>
    new Array(sectionCount).fill(true).map((value, index) => {
        const offset = index === sectionCount - 1 ? 0 : 1;
        return itemSize * (data[index].length + offset);
    })
);

const createList = memoize(
    (
        sectionCount,
        itemSizes,
        source,
        ...
        <AutoSizer key="list">
            {({height, width}) => (
                <VariableSizeList
                    ...
                    itemData={createSectionData(
                        source,
                        standalonesState[extProps.item],
                        classes,
                        itemSize,
                        width
                    )}
